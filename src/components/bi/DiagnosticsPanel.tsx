...
export function DiagnosticsPanel({ diagnostics }: { diagnostics: BoardDiagnostics }) {
  ...
  return (
    ...
    >
      ...
      {d.statusSemantics.length > 0 ? (
        <div className="mt-4">
          <p className="text-[11px] font-medium tracking-[0.09em] text-muted-foreground uppercase">
            Work-order status semantics (raw → operational)
          </p>
          <ul className="mt-2 space-y-1.5">
            {d.statusSemantics.map((r) => (
              <li key={r.raw} className="rounded-md border border-border bg-background/50 px-2.5 py-2 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate">
                    {r.raw}
                    <span className="text-muted-foreground"> → {r.operational}</span>
                    {r.commercial ? (
                      <span className="text-muted-foreground"> (commercial: {r.commercial})</span>
                    ) : null}
                  </span>
                ...
                <p className="mt-1 text-[11px] text-muted-foreground">{r.interpretation}</p>
              </li>
            ))}
          </ul>
        </div>
