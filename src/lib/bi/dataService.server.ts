...
export async function loadDataSet(options: LoadOptions = {}): Promise<DataSet> {
  ...
  // The configured board IDs are authoritative: MONDAY_DEALS_BOARD_ID is the
  // Deals board and MONDAY_WORK_ORDERS_BOARD_ID is the Work Orders board.
  // Each board is queried independently and mapped with its own schema.
  const dealsBoard = boardA;
  const woBoard = boardB;
